...
// Llama a la función de conexión y almacena el resultado en $mysqli
$mysqli = conexion();

include('layout/admin/datos_usuario_sesion.php');

// Verifica que la conexión mysqli se haya establecido
if (!$mysqli) {
...
$result_informacions = $mysqli->query($query_informacions);

$informacions = [];
if ($result_informacions) {
    while ($informacion = $result_informacions->fetch_assoc()) {
        $informacions[] = $informacion;
    }
}

$id_informacion = null; // Inicializar la variable
...
$query_facturaciones = "SELECT * FROM tb_facturaciones WHERE estado = '1'";
$result_facturaciones = $mysqli->query($query_facturaciones);

if ($result_facturaciones) {
    while ($facturacione = $result_facturaciones->fetch_assoc()) {
        $contador_del_nro_de_factura++;
    }
}
$contador_del_nro_de_factura++; // Aumentar el contador en 1
